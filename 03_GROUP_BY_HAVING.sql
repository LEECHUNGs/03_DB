...
-- 그룹함수는 단 한개의 결과값만 산출하기에 그룹이 여러개일 경우 오류 발생
...
FROM EMPLOYEE
GROUP BY DEPT_CODE; --> DEPT_CODE 가 같은 행끼리 하나의 그룹이됨
...
SELECT DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여') 성별, 
			 ...
			 SUM(SALARY) "급여 합" 
FROM EMPLOYEE
GROUP BY DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여') -- 별칭사용 X (SELECT 절 해석 X)
ORDER BY "인원 수"; -- 별칭사용O (SELECT절 해석 완료)


-- EMPLYOEE 테이블에서 부서코드가 D5, D6인 부서의 평균 급여, 인원 수 조회
SELECT DEPT_CODE,
			 ROUND(AVG(SALARY)),
			 COUNT(*)
FROM EMPLOYEE
...
FROM EMPLOYEE
-- WHERE SALARY >= 300000 -->한 사람의 급여가 3백만 이상이라는 조건(요구사항 X)
GROUP BY DEPT_CODE
...
FROM EMPLOYEE
GROUP BY JOB_CODE
HAVING COUNT(JOB_CODE) <= 5 -- HAVING절에는 그룹 함수가 반드식 작성된다!
ORDER BY JOB_CODE;
...
/* SET OPERATOR (집합 연산자) 
 * 
-- 여러 SELECT의 결과(RESULT SET)를 하느의 결과로 만드는 연산자
