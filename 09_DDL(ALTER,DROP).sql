...
-- 객체를 만들고(CREATE), 바꾸고(ARTER), 삭제하는(DROP) 데이터 정의 언어
...
-- DEPT_COPY의 DEPT_TITLE의 컬럼에 UNIQUE 추가
...
-- ORA-12899: "KH_LCS"."DEPT_COPY"."DEPT_ID" 열에 대한 값이 너무 큼(실제: 3, 최대값: 2)
...
--> 해결
-- 1) 자식, 부모테이블 순서로 삭제
-- 2) ALTER 를 이용해 FK 제약조건 삭제 후 TB1 삭제
