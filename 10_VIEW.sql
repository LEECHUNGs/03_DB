/* 
 * VIEW
 * - SELECT문의 실행결과 (RESULT SET)를 화면에 저장하는 객체
 * - 논리적 가상 테이블
 ...
 * 
 * -- 3) WITH CHECK OPTION : 옵션을 성정한 컬럼의 값을 수정 불가능하게 함.
 * 
...
-- 1) SYS 관리자 계정 접속
-- 2) ALTER SESSION SET "_ORACLE_SCRIPT = TRUE;" -- 이전 버전 문법 사용하게 해주는 코드
-- 3) GRANT CREATE VIEW TO kh_lcs;
-- 4) 권한 부여받은 kh 계정으로 접속하여 위 VIEW 생성구문 다시 실행

ALTER SESSION SET "_ORACLE_SCRIPT" = TRUE;

GRANT CREATE VIEW TO kh_lcs;
...
CREATE OR REPLACE VIEW V_DCOPY2
	AS SELECT OR REPLACE EMP_ID 사번, EMP_NAME 이름, DEPT_TITLE 부서명, JOB_NAME 직급명
		FROM EMPLOYEE
		JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
		JOIN JOB USING(JOB_CODE);
...
-- VIEW의 원래 목적인 보여지는 것(조회)이라는 용도에 맞게 사용하는게 좋다
