...
-- INSERT 하고자 하는 컬럼이 모든 컬럼일 경우 컬럼명 생략 가능.
-- 단, 컬럼의 순서를 지켜서 VALUES에 값을 기입해야 함.

INSERT INTO EMPLOYEE2
VALUES('900', '장채현', '901230-2345678', 'jang_ch@kh.or.kr'
				, '01012341234', 'D1', 'J7', 'S3', 430000, 0.2, 200, SYSDATE, NULL, 'N');
...
-- 선택 안된 컬럼은 값이 NULL이 들어감 (DEFAULT 전재 시 DEFAULT 값으로 삽입됨)
...
-- DEPARTMENT2 테이블에서 DEPT_ID가 'D9'인 행의 DEPT_TILTE을
-- '전략기획팀'으로 수정
...
-- DEPARTMENT2 테이블에서 BOUNS를 받지 않는 사원의
-- BONUS를 0.1로 변경
UPDATE EMPLOYEE2
SET BONUS = 0.1
WHERE BONUS IS NULL;
...
-- EMPLOYEE2 테이블에서
-- 방명수 사원의 급여와 보너스율르 유재식 사원과 동일하게 변경
...
-- 만약 WHERE에 조건을 설정하지 않으면 모든 행이 다 삭제됨
...
-- EMPLOYEE2 테이블에서 이름이 '장채현'인 사원 정보를 삭제
