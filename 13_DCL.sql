...
* 사용자 계정 : 데이터베이스에 대하여 질의, 갱신, 보고서 작성 등의
                ...
                ex) kh_lcs계정(각자 이니셜 계정), workbook 등
*/


-- GRANT RESOURCE, CONNECT TO kh_lcs;

-- 롤 (role)
-- 다수 사용자와 다양한 권한을 효율적으로 관리하기 위하여
-- 서로 관련된 권한을 그굽화난 개념(권한의 묶음)

SELECT * FROM ROLE_SYS_PRIVS -- 오라클 DB에서 시스템 권한을 가진 역할을 나타내는 데이터 딕셔너리
WHERE ROLE = 'RESOURCE';
...
CREATE USER lcs_sample IDENTIFIED BY sample1234;


-- 2. 새 연결 추가
--> 접속 권한( CREAT SESSION ) 없어서 오류 발생


-- 3. 접속 권한 부여
-- [권한 부여 작성법]
-- GRANT CREATE SESSION, 권한, 권한... TO 사용자명;
GRANT CREATE SESSION TO lcs_sample;
...
GRANT CREATE TABLE TO lcs_sample;


ALTER USER lcs_sample DEFAULT TABLESPACE
...
GRANT CONNECT, RESOURCE TO lcs_sample;

-- CONNECT : DB 접속 관련 권한을 묶어둔 ROLE
-- RESORCE : DB 사용을 위한 기본 객체 생성 권한을 묶어둔 ROLE
...
-- kh_lcs		/		lcs_sample 사용자 계정끼리 서로 객체 접근 권한 부여

-- 1. kh_lsc 계정의 EMPLOYEE 테이블 조회
SELECT * FROM KH_LCS.EMPLOYEE;
...
GRANT SELECT ON EMPLOYEE TO lcs_sample;
...
REVOKE SELECT ON EMPLOYEE FROM lcs_sample;
