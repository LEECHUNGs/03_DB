...
-- EMPLYOEE 테이블에서 모든 사원의 사번, 이름, 급여, 연봉(급여 * 12) 조회
SELECT EMP_ID, EMP_NAME, SALARY, SALARY * 12 FROM EMPLOYEE;


SELECT EMP_NAME + 10 FRMO EMPLOYEE;
-- ORA-01722 : 수치가 부적합합니다
-- 산술 연산은 숫자(NUMBER 타입)만 가능

-- 날짜(DATE) 타입 조회

-- EMPLYOEE 테이블에서 이름, 입사일, 오늘 날짜 조회
...
-- 주의 사항 1) DISTINCT 구문은 SELECT 마다 1번씩만 작성 가능
...
-- 3. SELECT 절 : SELECT ~ 컬럼명 까지
-- 1. FROM 절 : FROM ~ 테이블 명 까지
-- 2. WHERE 절 : WHERE 컬럼명 연산자 값;
-- SELECT * FROM EMPLYOEE WHERE EMP_ID = 300;

-- EMPLYOEE 테이블에서 급여가 3백만원 초과인 사원의
-- 사번, 이름, 급여, 부서코드를 조회
...
-- 오라클은 데이터 타입이 달라도 형변환 시켜서 일치하면 TRUE
...
-- 문제점 : _를 기준으로 삼았으나, 패턴 중 '_'와 동일한 표기법으로 작성되어
--				  구분되지 않음

-- 해결 방법 : LIKE에 ESCAPE OPTION 을 이용해 _를 구분
