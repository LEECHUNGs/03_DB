 ...
 * INDEX (색인)
 * 
 * - SQL 명령문 중 SELECT의 처리 속도를 향상 시키기 위ㅣ해
 * 컬럼에 대해서 생성하는 객체
 * 
 ...
 * - 조회 시 전체 테이블 내용을 조회하기 때문에
 *   시스템 부하가 낮아져 전체적인 성능 향상이 된다.
 * 
 * 
 * * 인덱스 단점
 * - 데이터 변경(INSWERT, UPDATE, DELETE)이 번번한 경우 오히려 성능이 저하되는 문제 발생
 * - 인덱스도 하나의 객체이기에 이를 저장하기 위한 별도 공간 생성 시간이 필요
 * 
 * 
 * -- 인덱스 생성 방법 --
 * 
