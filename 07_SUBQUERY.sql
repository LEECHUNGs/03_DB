 ...
 * 
 * - 다중열 서브쿼리 : 조회 결과 행 수와 열 수가 여러개일 때
 * 
 * - 상(호연)과 서브쿼리 : 서브쿼리가 만든 결과 값을 메인쿼리가 비교 연산할 때
 * 												 메인쿼리 테이블의 값이 변경되면 서브쿼리의 결과값도 바뀌는 서브쿼리
 * 
 * - 스칼라 서브쿼리 : 상관 쿼리이면서 결과 값이 하나인 서브쿼리
 * 
 * ** 서브쿼리 유형에 따라 서브쿼리 앞에 붇는 연산자가 다름 **
 * 
 * 
 * */
...
-- 3) 대리 직급의 직원들 중에 과장직급의 급여보다 많이 받는 직원
...
-- DEPARTMENT 테이블의 LOCATINO_ID와 동일한 DEPT_ID가
-- EMPLOYEE 테이블의 DEPT_CODE와 동일한 사원을 구하시오.
								
-- 1) LOCATION 테이블에서 NATIONAL_CODE가 KO인 경우
SELECT LOCAL_CODE
FROM LOCATION
WHERE NATIONAL_CODE = 'KO';

-- 2) DEPARTMENT 테이블의 LOCATINO_ID와 동일한 DEPT_ID
SELECT DEPT_ID
FROM DEPARTMENT
...
-- 2. 2000년도에 입사한 사원의 부서와 직급이 같은 사원을 조호
-- 		사번, 이름, 부서코드, 직급코드, 입사일
...
-- 4. 다중행 다중열 서브쿼리
--		서브쿼리의 조회 결과 행 수와 열 수가 여러개 일 때


-- 본일 직급의 평균 급여를 받고 있는 직원의
-- 사번, 이름, 직급, 급여를 조회
-- 단, 급여과 급여 평균은 만원단위로 계산 TRUNC(컬럼명, -4)
...
-- 5. 상[호연]관 서브쿼리
-- 상관 궈리는 메인쿼리가 사용하는 테이블값을 서브쿼리가 이용해서 결과를 만듦
-- 메인쿼리의 테이블값이 변경되면 서브커리의 결과값도 바뀌게 되는 구조

-- 상관쿼리는 먼저 메인쿼리 한 행을 조회하고
-- 해당 행이 서브쿼리에 조건을 충족하는지 확인하여 SELECT를 진행함
...
WHERE DEPT_CODE = 'D1';
;
/* 1. 메인쿼리의 첫번째 행인 바명수가 있는 부서 D1의 가장빠른 입사일
 * 		2007-03-20
...
--		서브쿼리가 만든 결과 집합(RESULT SET)을 테이블 대신에 사용한다.
...
SELECT DEPT_TITLE, DEPT_CODE, 평균급여
